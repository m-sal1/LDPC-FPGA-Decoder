# CCSDS LDPC decoder smoke tests
# Moustafa Salman
...
async def wait_for_done(dut, test_name, max_cycles=MAX_CYCLES):
    for cycles in range(1, max_cycles + 1):
        await RisingEdge(dut.clk)
        if cycles % 50000 == 0:
            cocotb.log.info(
                f"[{test_name}] cycle {cycles} | "
                f"done={int(dut.decoding_done.value)} | "
                f"converged={int(dut.converged.value)}"
            )
...
async def test_smoke_all_zero(dut):
    ...
    cycles, timed_out = await wait_for_done(dut, "smoke_all_zero")
    ...
    bits = await read_decoded_bits(dut)
    assert sum(bits) == 0, f"{sum(bits)} bit errors"
...
async def test_smoke_with_errors(dut):
    ...
    for pos in error_positions:
        llr_vector[pos] = -LLR_STRONG  # flip to indicate error
    ...
    cycles, timed_out = await wait_for_done(dut, "smoke_with_errors")
    assert not timed_out, f"FSM stuck after {MAX_CYCLES} cycles"

    await RisingEdge(dut.clk)
    converged = int(dut.converged.value)
    cocotb.log.info(f"Done at cycle {cycles}, converged={converged}")

    bits   = await read_decoded_bits(dut)
    errors = sum(bits)
    cocotb.log.info(f"Remaining errors: {errors}/512")

    assert converged == 1, f"Did not converge. Remaining errors={errors}"
    assert errors   == 0,  f"converged=1 but {errors} errors remain"
    cocotb.log.info("ERROR CORRECTION TEST PASSED")
